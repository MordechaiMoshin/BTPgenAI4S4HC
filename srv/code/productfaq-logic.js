...
/**
 * 
 * @After(event = { "CREATE","UPDATE" }, entity = "motymoshin_2_a2Srv.ProductFAQ")
...
module.exports = async function(results, request) {
	try {
		...
		let productFAQ = await SELECT.one('motymoshin_2_a2.ProductFAQ').where({ ID: productFAQID }).forUpdate();
		...
		await UPDATE("motymoshin_2_a2.ProductFAQ", ID).with({ embedding: utils.array2VectorBuffer(embedding) });
