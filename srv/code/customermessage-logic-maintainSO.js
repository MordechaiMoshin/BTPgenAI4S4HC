...
/**
 * 
 * @Before(event = { "Action2" }, entity = "motymoshin_2_a2Srv.CustomerMessage")
...
module.exports = async function(request) {
	try {
		...
		const customerMessage = await SELECT.one.from('motymoshin_2_a2.CustomerMessage').where({ ID });
		...
		if (attachedSOId) {
			...
			await UPDATE('motymoshin_2_a2.CustomerMessage')
